...
SystemStats Simulation::calculate_statistics()
{
    ...
    for (auto idx = this->processes.begin(); idx != this->processes.end(); idx++)
    {
        for (auto thread : idx->second->threads)
        {
            ioTime += thread->io_time;
            serviceTime += thread->service_time;
            priority = thread->priority;
            this->system_stats.thread_counts[priority]++;
            this->system_stats.avg_thread_response_times[priority] += thread->start_time - thread->arrival_time;
            this->system_stats.avg_thread_turnaround_times[priority] += thread->end_time - thread->arrival_time;

            if (thread->end_time > maxTime)
            {
                maxTime = thread->end_time;
            }
        }
    }
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            system_stats.avg_thread_response_times[i] = 0;
            system_stats.avg_thread_turnaround_times[i] = 0;
        }
    }
    this->system_stats.total_time = maxTime;
    this->system_stats.total_idle_time = this->system_stats.total_time - serviceTime - this->system_stats.dispatch_time;
    this->system_stats.service_time = serviceTime;
    this->system_stats.io_time = ioTime;
    this->system_stats.total_cpu_time = serviceTime + this->system_stats.dispatch_time;
    this->system_stats.cpu_utilization = 100 * double(this->system_stats.total_cpu_time) / double(this->system_stats.total_time);
    this->system_stats.cpu_efficiency = 100 * double(serviceTime) / double(this->system_stats.total_time);
    //system_stats.thread_counts = ;
